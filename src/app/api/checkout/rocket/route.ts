...
export async function POST(request: Request) {
  try {
    ...
    enforceRateLimit({
      key: `checkout:rocket:${getRequestIp(request)}`,
      limit: 10,
      windowMs: 60_000,
    });

    const payload = checkoutPayloadSchema.parse(await request.json());
    const paymentReference = `ROCKET-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    ...
    return jsonSuccess({
      orderId: order.id,
      paymentReference,
      url: `/account?orderCreated=true&orderId=${order.id}&paymentMethod=rocket`,
    });
  } catch (error) {
